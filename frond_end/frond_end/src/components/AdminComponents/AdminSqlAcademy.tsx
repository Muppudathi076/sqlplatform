...
import { Pencil, Trash2, Plus, X, Loader2, GraduationCap, Search, Download, Sparkles } from "lucide-react"
import ReusableTable from "../ReusableComponents/ReusableTable"
import toast from "react-hot-toast"
import {
  AcademyGetAllApi,
  addAcademyApi,
  updateAcademyByIdApi,
  deleteAcademyByIdApi,
  bulkDeleteAcademyApi,
  seedAcademyApi,
  generateAiAcademyApi
...
function AdminSqlAcademy() {
  const token = localStorage.getItem("access_token") || ""

  const [entries, setEntries] = useState<AcademyEntry[]>([])
  ...
  const fetching = async () => {
    setLoading(true)
    try {
      const response = await AcademyGetAllApi(token)
      setEntries(response.data || [])
    } catch (e) {
      console.log("API Error:", e)
      toast.error("Failed to load academy entries")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async () => {
    try {
      ...
      try {
        colsArr = JSON.parse(formData.columns)
        tableDataArr = JSON.parse(formData.tableData)
      } catch {
        toast.error("Columns and Table Data must be valid JSON arrays")
        return
      }
      ...
      if (isEdit && editId !== null) {
        await updateAcademyByIdApi(editId, token, payload)
        toast.success("Entry updated successfully")
      } else {
        await addAcademyApi(token, payload)
  ...
  const handleEdit = (entry: AcademyEntry) => {
    setFormData({
      title: entry.title,
      instruction: entry.instruction,
      expectedQuery: entry.expectedQuery || "",
      columns: JSON.stringify(entry.columns || [], null, 2),
      tableData: JSON.stringify(entry.tableData || [], null, 2),
      successMsg: entry.successMsg || "",
      hint: entry.hint || ""
    })
    setEditId(entry.id)
    setIsEdit(true)
  ...
  const handleDelete = async () => {
    try {
      if (deleteId !== null) {
        await deleteAcademyByIdApi(deleteId, token)
        await fetching()
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkDeleteAcademyApi(selectedRows, token)
  ...
  const handleSeed = async () => {
    setSeeding(true)
    try {
      const res = await seedAcademyApi(token)
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      const res = await generateAiAcademyApi(token, aiCount)
  ...
  const columns = [
    ...
    { header: "Query", accessor: "expectedQuery" },
  ]
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">Manage all SQL Academy lessons — add, edit, or remove missions.</p>
        </div>
      ...
      <motion.div variants={itemVariants} className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder="Search titles or instructions..."
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Meet Your Data"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Expected Query</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Columns (JSON Array)</label>
                    <textarea
                      placeholder='["id", "name"]'
                      value={formData.columns}
                      onChange={(e) => setFormData({ ...formData, columns: e.target.value })}
                      className="w-full border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white p-3 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all min-h-[100px] font-mono text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Table Data (JSON Array)</label>
                    <textarea
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Success Message</label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. Use the wildcard character *"
                      value={formData.hint}
          ...
          >
            ...
            >
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                Are you sure you want to delete this academy entry? This action cannot be undone.
              </p>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                Use Gemini AI to automatically generate new, unique SQL Academy interactive missions.
              </p>

              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Number of Missions to Generate: <span className="font-bold text-purple-600 dark:text-purple-400">{aiCount}</span>
                </label>
                <input
                  type="number"
                  min="1"
...
export default AdminSqlAcademy
